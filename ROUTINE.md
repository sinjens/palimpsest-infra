...
- **2026-08-25** (v0.8.1): Fixed a followups-queue monopoly. v0.8.0 cleared a followup only when its article was EDITED and re-showed shown-but-unedited items forever, so ~40 soft "verify current state" flags the reviewer kept declining to edit accumulated (passes_seen to 22), grew the queue to ~44, and ate 95% of the review budget — crushing the audit rotation from 30 to 1 article/night and deferring real changes. Fix: **clear-on-shown** (being pulled into a full review IS the adjudication — drop it edited or not; the reviewer re-emits if a real issue persists) + a **hard cap** (`PALIMPSEST_FOLLOWUP_MAX_PER_NIGHT`, default 12) so the queue can never again monopolise, with dirty ordered before followups and the audit guaranteed its slice. Items that can't win a review slot for `FOLLOWUP_ESCALATE_AFTER` nights are dropped with a loud warning. Verified on the live work brain: audit restored 1→30/night, followups 42→12.
- **2026-07-29** (v0.8.0): Closed the supervise correction loop. Diagnosis (a peer agent, verified in code): supervise could *detect* that an article states something now-false but had no way to *act* on it — findings about articles not shown in full dead-ended in the changelog (nothing re-read them), neighbour expansion was outbound-only (an article referencing a changed one never got pulled in), and the staleness rule was gated on TTL expiry so `stable`/long-TTL articles were never re-examined for falsehood. Fixes: (1) a durable **followups queue** (`compile/followups.json`) — the supervisor emits `@@@FOLLOWUP` blocks for unshowable articles, which are re-seeded into later reviews and cleared only when the article is shown AND edited, escalating loudly after `PALIMPSEST_FOLLOWUP_ESCALATE_AFTER` passes; (2) **bidirectional neighbours** — articles that reference a changed one (via `related:` or body text) are now pulled into review too; (3) prompt: "contradicts reality/newer article" is correctable regardless of TTL (TTL is a review trigger, not a gate); (4) synthesis prompt: when new evidence contradicts a recorded fix/decision, EDIT the original claim (or mark it superseded) rather than appending a co-equal dated section that leaves the falsehood standing.
- **2026-07-22** (v0.7.2): Scale hardening after the work brain (686 articles) stalled. Supervise had ballooned to reviewing 362 articles / ~478k tokens in one call (the audit was a fixed 1/7 *fraction* that grew with the library, and a failed review never advanced its frontier so the dirty set accumulated nightly into a spiral) — it now caps the review to `PALIMPSEST_REVIEW_TOKEN_BUDGET` (~120k), makes the audit a fixed *count* (`PALIMPSEST_AUDIT_COUNT`, default 30) rotating through the whole library, and always advances the frontier so the backlog drains instead of spiralling. Compile stopped letting one bad session wedge the whole run: a session whose synthesis fails (or exceeds an optional `PALIMPSEST_MAX_SESSION_BYTES` ceiling) is recorded to `compile/failed-sessions.log`, the cursor advances past it, and the run continues — re-compile one later with `main.py --date <d> --session <id>`.
